...
class TestOperationTypes(unittest.TestCase):
    """Test suite for Pydantic operation types and helpers."""
    ...
    def test_add_clip_op_track_kind_must_be_video_or_audio(self) -> None:
        with pytest.raises(ValidationError):
            AddClipOp(
    ...
    def test_add_transition_op_type_must_be_valid(self) -> None:
        with pytest.raises(ValidationError):
            AddTransitionOp(
                author="ai", clip_a_id="c1", clip_b_id="c2",
    ...
    def test_project_has_assets_and_edit_graph(self) -> None:
        p = Project(name="test")
        assert p.assets == {}
        assert p.edit_graph == []
        assert p.project_id
