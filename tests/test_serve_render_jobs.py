...
``open_edit.kernel.render_jobs.RenderJobService``
(``DEFAULT_RENDER_JOB_SERVICE``) into each project's
...
running → completed) through the RenderJobService API, which is the
...
async def test_render_job_lifecycle_queued_running_succeeded(tmp_path):
    ...
    asserted, now at the DB level through RenderJobService."""
    ...
    with mock.patch(
        ...
        try:
            ...
            running = DEFAULT_RENDER_JOB_SERVICE.get(tmp_path, job.job_id)
            assert running is not None
            assert running.status == "running"

            release.set()
            final = await DEFAULT_RENDER_JOB_SERVICE.wait(tmp_path, job.job_id)
...
async def test_enqueue_coalesces_duplicate_jobs(tmp_path):
    ...
    with mock.patch(
        ...
        try:
            first = DEFAULT_RENDER_JOB_SERVICE.enqueue("proj", tmp_path, "proxy")
            for _ in range(4):
                dup = DEFAULT_RENDER_JOB_SERVICE.enqueue("proj", tmp_path, "proxy")
                assert dup.job_id == first.job_id

            active = [
                j for j in DEFAULT_RENDER_JOB_SERVICE.list_jobs(tmp_path)
                if j.status in ("queued", "running")
            ]
            assert len(active) == 1, "duplicate enqueues must coalesce"

            release.set()
            final = await DEFAULT_RENDER_JOB_SERVICE.wait(tmp_path, first.job_id)
            assert final.status == "succeeded"
        finally:
            release.set()
