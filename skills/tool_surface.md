...
`open_edit/serve/tool_schemas.py` — `TOOL_USAGE_GUIDE`. If anything
in this file disagrees with `TOOL_USAGE_GUIDE`, the guide wins.

## Relevant source (read these in the real codebase)

- `open_edit/serve/tool_schemas.py` — `TOOL_USAGE_GUIDE` (authoritative)
- `open_edit/serve/pillar_tools.py` — the dispatch layer:
  - `dispatch_query` (maps the 6 query names)
  - `dispatch_edit` (`add_marker` / `set_pinned_value` / `import_asset` / `ingest_local` / `apply_generated_ops`)
  - `dispatch_generate` (`sfx` / `music` / `visual` / `silence_cuts`)
